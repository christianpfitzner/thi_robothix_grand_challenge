...
class MoveItArmInterface
{
  public:
    ...
    void moveToFrameLinear(std::string frame_id, double z_offset_m = 0, double z_orientation_rad = 0, EE_LINKS ee_link=EE_LINKS::PANDA_HAND_TCP)
    {
        ...
        case EE_LINKS::PANDA_PROBE:
            offset.header.frame_id = "panda_hand_tcp";
            offset = tfBuffer_.transform(offset, "panda_hand_probe");
            break;
        ...
        offset = tfBuffer_.transform(offset, "panda_link0", ros::Duration(1.0));
  ...
  private:
    ...
    void moveToFrameLinear(std::string frame_id, geometry_msgs::Pose & offset)
    {
        ...
        mgi_->limitMaxCartesianLinkSpeed(0.1);
        // mgi_->lim
        ...
        // for(int i=1; i<21; i++)
        // {
        //     mgi_->setMaxAccelerationScalingFactor(0.1/i);
        //     // mgi_->setMaxVelocityScalingFactor(0.1/i);
        //     if (mgi_->plan(my_plan_arm_) == moveit::planning_interface::MoveItErrorCode::SUCCESS)
        //         break;
        //     else
        //     {
        //         ROS_WARN("MoveItArmInterface::moveToFrameLinear: Failed to plan to target pose");
        //         if(i==20)
                    // throw std::runtime_error("MoveItArmInterface::moveToFrameLinear: Failed to plan to target pose");
        //     }
        // 
        ...
        // int i = 0;
        // while(mgi_->plan(my_plan_arm_) != moveit::planning_interface::MoveItErrorCode::SUCCESS)
        // {
        //     mgi_->setMaxAccelerationScalingFactor(0.1/i);
        //     if (i>5)
        //     {
